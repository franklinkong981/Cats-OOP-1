...
class Dog {
  ...
  static async create(new_name, new_age) {
    const results = await db.query(`INSERT INTO dogs (name, age) VALUES ($1, $2) RETURNING id, name, age`, [new_name, new_age]);
    const {id, name, age} = results.rows[0];
    return new Dog(id, name, age);
  }
  async remove() {
    const results = await db.query(`DELETE FROM dogs WHERE id = $1`, [this.id]);
  }
  async save() {
